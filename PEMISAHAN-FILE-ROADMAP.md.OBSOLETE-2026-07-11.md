# ⚠️ ARSIP — DOKUMEN INI SUDAH BASI, JANGAN DIPAKAI SEBAGAI ACUAN

> **Status: OBSOLETE per 2026-07-11.** Dipindah ke `archive/` supaya tidak
> kebaca sebagai rencana aktif oleh sesi Claude berikutnya.
>
> Dokumen di bawah ini nyebut file-file (`features-etalase-piutang-renovai.js`,
> `features-gaji-cobek-tagihan.js`, `features-renovasi-pajak-aset-order.js`,
> dst) sebagai "belum dipecah / rencana ke depan" — kenyataannya file-file
> itu **SUDAH TIDAK ADA**, sudah dipecah jadi file dedicated
> (`cobek.js`, `aset.js`, `piutang-utang.js`, `renovasi.js`, `gaji-calc.js`,
> `tagihan-kalender.js`, `akun.js`, `kategori.js`, dan lain-lain) di
> sesi-sesi kerja setelah dokumen ini ditulis, tapi dokumennya sendiri tidak
> pernah diupdate — jadi isinya menyesatkan kalau dibaca sebagai status
> terkini.
>
> **Sumber kebenaran yang benar & selalu akurat untuk "file mana isinya
> apa":** `FILE-MAP.md` di root project (di-generate otomatis dari source
> lewat `node scripts/generate-file-map.js`, dijalankan otomatis tiap
> `node build.js`) — bukan dokumen prosa manual seperti ini yang gampang
> basi begitu ada sesi lain yang kerja tanpa update baliknya.
>
> Riwayat/histori roadmap split file (untuk konteks historis, BUKAN status
> terkini) tetap ada di `CLAUDE.md` (catatan kerja tiap sesi, bagian ke-1
> s/d ke-11 dst) — itu juga lebih bisa dipercaya karena tiap entrinya
> mencantumkan hasil verifikasi nyata (test/browser), bukan rencana.
